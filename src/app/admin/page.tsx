...
export default function AdminDashboard() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    setFetchError(null)
    try {
      const data = await getAdminReports(filterStatus)
  ...
  const selectedReportRating = selectedReport?.rating ?? 0
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-20 text-center">
                    <div className="w-16 h-16 bg-danger-light rounded-3xl flex items-center justify-center mx-auto mb-4">
                      <AlertTriangle className="w-8 h-8 text-danger" />
                    </div>
                    <p className="text-sm font-bold text-foreground">Gagal Memuat Laporan</p>
                    <p className="text-xs text-muted mt-1">{fetchError}</p>
                    <button
                      onClick={fetchReports}
                      className="mt-5 px-5 py-2.5 rounded-xl bg-primary text-white text-xs font-black uppercase tracking-widest hover:bg-primary-dark transition-all"
                    >
                      Coba Lagi
                    </button>
                  </td>
                    ...
                    <tr key={report.id} className="hover:bg-muted-light/30 transition-colors group">
                      ...
                      <td className="px-6 py-5 max-w-xs">
                        <div className="flex items-center gap-2 text-xs font-medium text-muted leading-tight truncate">
                          ...
                          {report.address}
                          {report.risk_flag && (
                            <span className="ml-2 flex-shrink-0 bg-danger-light text-danger px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest">
                              ⚠️ Risk Flag
                            </span>
                          )}
                        </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-muted">GPS Kejadian (Penentu Wilayah)</p>
                    <div className="grid grid-cols-2 gap-2 rounded-2xl border border-border bg-white p-4 text-[11px] font-bold text-foreground">
                      <span className="text-muted">Koordinat</span>
                      <span className="text-right">
                        {(selectedReport.latitude ?? selectedReport.lat).toFixed(6)}, {(selectedReport.longitude ?? selectedReport.lng).toFixed(6)}
                      </span>
                      <span className="text-muted">Akurasi</span>
                      <span className="text-right">
                        {selectedReport.gps_accuracy === null ? '-' : `${Math.round(selectedReport.gps_accuracy)}m`}
                      </span>
                      <span className="text-muted">Kelurahan</span>
                      <span className="text-right">{selectedReport.kelurahan ?? '-'}</span>
                      <span className="text-muted">Kecamatan</span>
                      <span className="text-right">{selectedReport.kecamatan ?? '-'}</span>
                      <span className="text-muted">Kab/Kota</span>
                      <span className="text-right">{selectedReport.kabupaten ?? '-'}</span>
                      <span className="text-muted">Provinsi</span>
                      <span className="text-right">{selectedReport.provinsi ?? '-'}</span>
                    </div>
                  </div>

                  <div className="rounded-2xl border border-accent/20 bg-accent-light/10 p-4">
                    <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-accent-dark flex items-center gap-1.5">
                      <AlertTriangle className="w-3.5 h-3.5" />
                      Audit Only (Tidak Mempengaruhi Wilayah)
                    </p>
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-bold text-foreground">
                      <span className="text-muted">GPS Pelapor</span>
                      <span className="text-right">
                        {selectedReport.reporter_lat ? `${selectedReport.reporter_lat.toFixed(6)}, ${selectedReport.reporter_lon?.toFixed(6)}` : '-'}
                      </span>
                      <span className="text-muted">Akurasi Pelapor</span>
                      <span className="text-right">
                        {selectedReport.reporter_accuracy ? `${Math.round(selectedReport.reporter_accuracy)}m` : '-'}
                      </span>
                      <span className="text-muted">Jarak ke TKP</span>
                      <span className="text-right">
                        {selectedReport.jarak_pelapor_km ? `${selectedReport.jarak_pelapor_km.toFixed(2)} km` : '-'}
                      </span>
                    </div>
                    {selectedReport.risk_flag && (
                      <div className="mt-3 p-3 bg-danger-light/50 rounded-xl border border-danger/20 flex flex-col gap-1">
                        <span className="text-[10px] font-black text-danger uppercase">⚠️ Risk Flag Terdeteksi</span>
                        <span className="text-xs text-danger-dark font-medium">{selectedReport.risk_note}</span>
                        <span className="text-[10px] text-danger-dark/70 mt-1">Laporan tetap valid namun disarankan review manual.</span>
                      </div>
                    )}
                  </div>
                  ...
                  {(selectedReport.rating || selectedReport.feedback_comment) ? (
                    <div>
                      <p className="mb-2 text-[10px] font-black uppercase tracking-widest text-muted">Rating & Feedback</p>
                      <div className="rounded-2xl border border-border bg-white p-4">
                        {selectedReport.rating ? (
                          <div className="flex items-center gap-1.5 text-accent-dark">
                            {Array.from({ length: 5 }).map((_, index) => (
                              <Star
                                key={`admin-rating-${index}`}
                                className={cn(
                                  'h-4 w-4',
                                  index < selectedReportRating ? 'fill-current text-accent' : 'text-border'
                                )}
                              />
                            ))}
                            <span className="ml-1 text-xs font-bold text-foreground">
                              {selectedReportRating}/5
                            </span>
                          </div>
                        ) : null}
