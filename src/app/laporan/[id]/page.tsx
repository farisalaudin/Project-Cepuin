        ...
        <section className="overflow-hidden rounded-[30px] border border-border bg-white shadow-[0_25px_85px_-48px_rgba(15,23,42,0.55)]">
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="space-y-5 p-5 sm:p-6">
              ...
              {(report.rating || report.feedback_comment) ? (
                <div className="rounded-2xl border border-border bg-white p-4">
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted/70">
                    Rating & Feedback
                  </p>
                  {report.rating ? (
                    <div className="mt-3 flex items-center gap-1.5 text-accent-dark">
                      {Array.from({ length: 5 }).map((_, index) => (
                        <Star
                          key={`detail-rating-${index}`}
                          className={cn(
                            'h-4 w-4',
                            index < reportRating ? 'fill-current text-accent' : 'text-border'
                          )}
                        />
                      ))}
                      <span className="ml-1 text-xs font-bold text-foreground">{reportRating}/5</span>
                    </div>
                  ) : null}
                  {report.feedback_comment ? (
                    <p className="mt-3 text-sm leading-relaxed text-foreground">{report.feedback_comment}</p>
                  ) : null}
                </div>
              ) : null}
